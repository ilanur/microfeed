...
export default class FeedCrudManager {
  ...
  _publicToInternalSchemaForBlogPost(blogPost) {
    const internalSchema = {};

    if (blogPost.title) {
      internalSchema.title = blogPost.title;
    }

    if (blogPost.content) {
      internalSchema.content = blogPost.content;
    }

    if (blogPost.author) {
      internalSchema.author = blogPost.author;
    }

    if (blogPost.publishedDate) {
      internalSchema.publishedDate = blogPost.publishedDate;
    }

    return internalSchema;
  }

  async upsertBlogPost(blogPost) {
    const postId = blogPost.id ? blogPost.id : randomShortUUID();
    this.feedContent.blogPost = {
      ...this._publicToInternalSchemaForBlogPost(blogPost),
      id: postId,
    };
